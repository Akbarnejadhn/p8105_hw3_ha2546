...
**ungroup did not work**

``` r
# Ex_spagettiplot = brfss_data %>% 
#   filter(
#     response == "Excellent"
#   ) %>% 
#   group_by(locationabbr) %>% 
#   summarize(
#     mean_location_data = mean(data_value)
#   ) %>% 
#   ungroup() %>% 
#   select(year, locationabbr, mean_location_data)
```

**geom\_smooth doesn’t work** 2010, distribution of data\_value for
...
graph_2006 = brfss_data %>%
  ...
  labs(
    title = "Distribution of Data Values for Response Levels in NY Counties",
    x = "Response Level",
    y = "Data Value",
    caption = NULL
  ) +
